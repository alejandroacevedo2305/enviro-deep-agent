...
def sample_failed_files(
    ...
    if not failed_files:
        print("❌ No FAILED-*.md files found in ParsedFiles/")
        return 0

    print(f"📂 Found {len(failed_files)} failed files in ParsedFiles/")

    # Determine sample size
    if all_files:
        sample = failed_files
        print(f"📋 Copying all {len(sample)} failed files...")
    else:
        actual_size = min(sample_size, len(failed_files))
        sample = random.sample(failed_files, actual_size)
        print(f"🎲 Randomly sampling {actual_size} failed files...")
    ...
    for src_file in sample:
        ...
        try:
            ...
            print(f"  ✗ Failed to copy {src_file.name}: {exc}")
...
def clear_samples(target_dir: Path) -> int:
    ...
    if not target_dir.exists():
        print("📂 Samples directory doesn't exist (nothing to clear)")
        return 0

    files = list(target_dir.glob("FAILED-*.md"))
    if not files:
        print("📂 No failed files in samples directory")
        return 0

    print(f"🗑️  Removing {len(files)} files from samples directory...")
    removed = 0
    for file in files:
        try:
            file.unlink()
            ...
            print(f"  ✗ Failed to remove {file.name}: {exc}")
...
Examples:
  ...
  uv run python SimpleWorkflow/sample_fails.py --clear
    ...
    if args.clear:
        ...
        print()
        print("=" * 70)
        print(f"✓ Removed {removed} files from samples directory")
        print("=" * 70)
        return 0
    ...
    print()
    print("=" * 70)
    print(f"✓ Successfully copied {copied} files to {target_dir}")
    print()
    print("📝 You can now:")
    ...
    print()
    print("💡 To clear samples later, run:")
